...
// import { userInfo } from "os";
	...
	onMount(async ()=>{
		...
		const res = await fetch(loginPath+'/panel/subject_get/'+subjectId,{mode:'cors',method:'get',headers:{'Authorization':'Bearer '+token}});
		if(res.status==200){
			try{
...
<div class="container w3-card " style="--template-color:red">
	{#if subject}
	<h1 class="w3-large w3-round w3-padding w3-margin ">Subject</h1>
	...
	<div class=" w3-padding w3-round">
		<div class="item">
			<label>Name</label>
			<p class="w3-large w3-text-blue">{subject.name}</p>
			<label>Stream Name</label>
			<p class="w3-large w3-text-blue">
				{#each subject.streams as stream}
              		{stream.name},
            	{/each}
			</p>
			<!-- <label></label>
			<p class="w3-large w3-text-blue">{subject.contact}</p> -->
		</div>
