...
<script>
	import BranchesList from '../../lib/components/lists/subjects.svelte';
...
// import { roomId } from '../room/[slug].svelte';
	...
	var subjects = null;
	 ...
	 onMount(async ()=>{
		...
		const res = await fetch(loginPath+'/panel/subjects'+classQuery,{mode:'cors',method:'get',headers:{'Authorization':'Bearer '+token}});
		if(res.status==200){
			try{
...
<div class="w3-round">
	{#if subjects}
		<!-- <p>sad</p> -->
		<BranchesList {subjects} />
	{:else}
		<p>No Subject Present</p>
	{/if}
</div>