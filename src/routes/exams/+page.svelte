...
<script>
	import ExamList from '../../lib/components/lists/exams.svelte';
...
// import { roomId } from '../room/[slug].svelte';
	...
	onMount(async ()=>{
		...
		Token.set(token);
		const res = await fetch(loginPath+'/panel/exams/'+classQuery,{mode:'cors',method:'get',headers:{'Authorization':'Bearer '+token}});
		if(res.status==200){
			try{
