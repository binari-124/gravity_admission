...
console.log("schemeID from slug:" + schemeId); 
	...
	async function getBody(){
		...
		let res;
			...
			res = await fetch(loginPath+'/panel/marking_scheme_get/'+schemeId,{mode:'cors',method:'get',headers:{'Authorization':'Bearer '+token,'Content-Type':'application/json'}});
	...
	async function handleSubmit(){
		...
		var loginPath=get(ApiUrl);
		let res;
		res = await fetch(loginPath+'/panel/marking_scheme_edit',{mode:'cors',method:'post',headers:{'Authorization':'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify(body)});
...
<h4 class="w3-black w3-round w3-card w3-padding">Edit Marking Scheme</h4>
