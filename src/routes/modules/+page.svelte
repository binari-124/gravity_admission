...
<script>
	import ModulesList from '../../lib/components/lists/modules.svelte';
...
// import { roomId } from '../room/[slug].svelte';
	...
	var modules = null;
	 ...
	 onMount(async ()=>{
		...
		const res = await fetch(loginPath+'/panel/modules'+classQuery,{mode:'cors',method:'get',headers:{'Authorization':'Bearer '+token}});
		if(res.status==200){
			try{
