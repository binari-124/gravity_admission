<script>
	...
	import StaffList from '../../lib/components/lists/staff.svelte';
...
// import { roomId } from '../room/[slug].svelte';
	...
	var staff = null;
	 ...
	 onMount(async ()=>{
		...
		const res = await fetch(loginPath+'/panel/staff_members',{mode:'cors',method:'get',headers:{'Authorization':'Bearer '+token}});
		if(res.status==200){
			try{
