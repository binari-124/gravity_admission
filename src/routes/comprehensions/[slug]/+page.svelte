...
import QuestionsList from '../../../lib/components/lists/questions.svelte';
	...
	onMount(async ()=>{
		...
		let query = new URLSearchParams({questions:true});
		const res = await fetch(loginPath+'/panel/comprehension_get/'+comprehensionId+"?"+query,{mode:'cors',method:'get',headers:{'Authorization':'Bearer '+token}});
		if(res.status==200){
			try{
					...
					if(response.status == "success")
					{
						...
						comprehension = response.data;
						// comprehension.content = atob(comprehension.content);
					}		
			}
			catch(e){
				console.log("caught");
	...
	async function handleDelete(){
		...
		var loginPath=get(ApiUrl);
		let res;
		res = await fetch(loginPath+'/panel/comprehension_delete/'+comprehensionId,{mode:'cors',method:'post',headers:{'Authorization':'Bearer '+token,'Content-Type':'application/json'}});
...
<div class="container " style="--template-color:red">
	{#if comprehension}
	Comprehension:
	<h1 class="w3-large w3-round w3-margin "> {comprehension.name}</h1>
	...
	<table class="w3-table-all w3-hoverable"  width="100%">
		...
		<tr class="w3-border">
			...
			<td><a class="w3-border w3-margin w3-round w3-hover-shadow w3-text-red w3-center" href="/comprehensions/edit/{comprehension._id}">Edit</a></td>
			<td><a class="w3-border w3-margin w3-round w3-hover-shadow w3-text-red w3-center w3-cursor-pointer" on:click={()=>{handleDelete()}} >Delete</a></td>
		</tr>
		<!-- {/each} -->
	  </table>

	  <hr class="w3-margin" />
	  <QuestionsList questions={comprehension.questions} />
	{:else}
	<!-- <p>comprehension Not found</p> -->
