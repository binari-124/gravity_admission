<script>
	...
	async function handleSubmit(){
		...
		var loginPath=get(ApiUrl);
		let res;
		res = await fetch(loginPath+'/panel/comprehension_create',{mode:'cors',method:'post',headers:{'Authorization':'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify(body)});
...
<h4 class="w3-black w3-round w3-card w3-padding"> Add a Comprehension </h4>
<form on:submit|preventDefault={handleSubmit} >
	...
	<!-- <input class="w3-input w3-border w3-round" type="date" bind:value={body.content} placeholder="Exam Date(*)" required/> -->
