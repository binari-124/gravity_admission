...
export let data;
export let topicId = data.topicId;
console.log("topicID from slug:" + topicId);	
	...
	async function handleSubmit(){
		...
		else
		{
			...
			res = await fetch(loginPath+'/panel/subtopic_create/'+topicId,{mode:'cors',method:'post',headers:{'Authorization':'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify(body)});
		...
		if(res.status==200){
			try{
					...
					{
						location.href="/chapters/topics/"+topicId;
					}
					else{
