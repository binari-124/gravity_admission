...
console.log("batchID from slug:" + batchId);
	...
	async function getBody(){
		var token = localStorage.getItem("token");
		var loginPath=get(ApiUrl);
		let res = await fetch(loginPath+'/panel/batches/batch_get/'+batchId,{mode:'cors',method:'get',headers:{'Authorization':'Bearer '+token,'Content-Type':'application/json'}});
		if(res.status==200){
			try{
	...
	async function handleSubmit(){
		...
		var loginPath=get(ApiUrl);
		let res;
		res = await fetch(loginPath+'/panel/batches/batch_edit/'+batchId,{mode:'cors',method:'post',headers:{'Authorization':'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify(body)});
...
<h4 class="w3-black w3-round w3-card w3-padding">Edit Batch</h4>
{#if body}
