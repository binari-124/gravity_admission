...
<script>
	import BatchesList from '../../lib/components/lists/batches.svelte';
...
// import { roomId } from '../room/[slug].svelte';
	...
	var batches = null;
	 ...
	 onMount(async ()=>{
		...
		const res = await fetch(loginPath+'/panel/batches'+classQuery,{mode:'cors',method:'get',headers:{'Authorization':'Bearer '+token}});
		if(res.status==200){
			try{
