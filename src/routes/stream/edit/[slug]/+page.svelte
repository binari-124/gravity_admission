<!-- <script context="module">
	export async function preload(p,session) {
		let params = p.params;
		return {streamId:params.slug};
	}
</script> -->
<script>
	/** @type {import('./$types').PageData} */
export let data;
export let streamId = data.streamId;
console.log("streamID from slug:" + streamId); 
	...
	async function getBody(){
		...
		let res;
			...
			res = await fetch(loginPath+'/panel/stream_get/'+streamId,{mode:'cors',method:'get',headers:{'Authorization':'Bearer '+token,'Content-Type':'application/json'}});
	...
	async function handleSubmit(){
		...
		var loginPath=get(ApiUrl);
		let res;
		res = await fetch(loginPath+'/panel/stream_edit',{mode:'cors',method:'post',headers:{'Authorization':'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify(body)});
...
<h4 class="w3-black w3-round w3-card w3-padding">Edit Stream</h4>
<form on:submit|preventDefault={handleSubmit} >
	<input class="w3-input w3-border w3-round" type="text" bind:value={body.name} placeholder="Name(*)" required/>
	<input class="w3-input w3-border w3-round" type="text" bind:value={body.description} placeholder="Description" required/>
