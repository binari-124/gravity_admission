...
// import { userInfo } from "os";
	...
	onMount(async ()=>{
		...
		const res = await fetch(loginPath+'/panel/stream_get/'+streamId,{mode:'cors',method:'get',headers:{'Authorization':'Bearer '+token}});
		if(res.status==200){
			try{
...
<div class="container w3-card " style="--template-color:red">
	{#if stream}
	<h1 class="w3-large w3-round w3-padding w3-margin ">Stream</h1>
	<a href="/stream/edit/{stream._id}">
	...
	<div class=" w3-padding w3-round">
		...
		<div class="item">
			<label>Name</label>
			<p class="w3-large w3-text-blue">{stream.name}</p>
			<label>Description</label>
			<p class="w3-large w3-text-blue">{stream.description}</p>
			<!-- <label>Contact</label>
			<p class="w3-large w3-text-blue">{stream.contact}</p> -->
		</div>
