...
<script>
	import FacultiesList from '../../lib/components/lists/faculties.svelte';
...
// import { roomId } from '../room/[slug].svelte';
	...
	var faculties = [];
	 ...
	 onMount(async ()=>{
		...
		const res = await fetch(loginPath+'/panel/faculties',{mode:'cors',method:'get',headers:{'Authorization':'Bearer '+token}});
		if(res.status==200){
			try{
