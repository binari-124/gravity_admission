<script>
	...
	function initializeTinyMice(){
	 ...
	 onMount(async ()=>{
		...
		body.questions.map(qna=>{
			questions.push(qna.question._id);
		});
    ...
    async function eventFunction(question,isInserted){
        ...
        if(isInserted)
        {
            body.questions.push({question,scheme:preferredScheme!="-"?preferredScheme:schemes[0]._id,subject:preferredSubject!="-"?preferredSubject:subjects[0]._id,instructions:preferredInstructions!="-"?preferredInstructions:questionInstructions[0]._id});
        }
        else
        {
			for(var i = 0; i <body.questions.length;i++)
			{
				if(body.questions[i].question._id == question._id)
				{
		...
		<div>
            ...
            <table class="w3-table-all w3-hoverable"  width="100%">
                ...
                <tr class="w3-hover-shadow">
                    <td>{@html atob(questionAndMarking.question.question)}</td>
