<script>
	...
	import ImageUpload from '../imageUpload.svelte.js';
...
import { bind, prevent_default } from 'svelte/internal';
	...
	async function getComprehensions(){
		// console.log(body);
		var token = localStorage.getItem("token");
		var loginPath=get(ApiUrl);
		let res;
		res = await fetch(loginPath+'/panel/comprehensions',{mode:'cors',method:'get',headers:{'Authorization':'Bearer '+token,'Content-Type':'application/json'}});
...
<h3 class="w3-black w3-round w3-padding width">{edit?"Edit a":"Create a"} <span class="w3-text-grey"> Single Choice Question</span></h3>
<div class="width">
	{#if edit}
	...
	<form on:submit|preventDefault={saveQuestion}>
		...
		{#if chapters  && chapters.length>0}
			...
			<select id="question_topic" class="w3-input w3-border w3-green w3-round w3-margin" bind:value={body.chapter} on:change={getTopics(body.chapter)}>
				<option  value="-">Chapter(*)</option>
				
				{#each chapters as chapter}
					<option class="w3-green"  value="{chapter._id}">{chapter.name}</option>
				{/each}
			</select>
			{#if topics && topics.length>0}
			<label for="question_topic">Topic</label>
			<select id="question_topic" class="w3-input w3-border w3-green w3-round w3-margin" bind:value={body.topic} on:change={getsubTopics(body.topic)}>
				...
				<select id="question_topic" class="w3-input w3-border w3-green w3-round w3-margin" bind:value={body.subtopic}>
		...
		<select id="question_difficulty" class="w3-input w3-border w3-green w3-round w3-margin" bind:value={body.difficulty}>
		...
		{#if comprehensions}
		<div class="w3-dropdown-hover w3-card  w3-round w3-margin">
			<p class="w3-button">Comprehensions</p>
			<div class="w3-dropdown-content w3-bar-block w3-border ztop">
				{#each comprehensions as comprehension}
				<!-- {console.log(exam.name)} -->
				<p class="w3-button w3-center w3-red w3-right" on:click={clearExam}>Clear</p>
				<div class="w3-margin">
					<span class="label">{comprehension.name}</span><input class="" type=radio value={comprehension._id} bind:group={body.comprehension} />	
				</div>
				{/each}
			</div>		
		  </div> 
