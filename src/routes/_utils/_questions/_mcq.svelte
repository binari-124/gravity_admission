...
<h3 class="w3-black w3-round w3-padding width">{edit?"Edit a":"Create a"} <span class="w3-text-grey">a Multiple Choice Question</span></h3>
<div class="width">
	{#if edit}
	...
	<form on:submit|preventDefault={saveQuestion}>
		...
		<input type="radio" checked={body.partial_marking} on:click={toggleMarkingPartial} /><br>
		<label for="question_correct"> Correct Answer</label><br>
		{#if body.answers}
			{#each ['A','B','C','D'] as a}
				<label>{a}</label>
					<input class="w3-checkbox w3-input" type="checkbox" value={a} bind:group={body.answers} />
			{/each}
		{/if}
