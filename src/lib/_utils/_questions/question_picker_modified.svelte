...
// import { userInfo } from "os";
	...
	import Questions from '../../../components/Lists/questions_modified.svelte';
    var chapters = null;
	var topics = null;
	var subtopics = null;

	var streams=[],subjects=[];
    ...
    async function getCategories()
    {
        var res;
        var token = localStorage.getItem("token");
        res = await fetch(loginPath+'/panel/categories',{mode:'cors',method:'get',headers:{'Authorization':'Bearer '+token}});
  ...
  {#if questions}
	...
	<div class="flex">
		...
		{#await forceUpdate(questions) then _}
			<Questions {picker} bind:selectedQuestions={selectedQuestions} questions={questions} />
		{/await}
