...
import { bind, prevent_default } from 'svelte/internal';
	...
	function initializeTinyMice(){
	 ...
	 onMount(async ()=>{
		...
		// await getChapters(null,false);
			...
			body.range_start = body.correct_answer.split("::")[0];
			body.range_end = body.correct_answer.split("::")[1];
	...
	async function saveQuestion(){
		...
		data.question_subtype = "range";
		
		

		try{
			if((!body.range_start || isNaN(parseInt(body.range_start)) || !body.range_end || isNaN(parseInt(body.range_end)))){
				alert("Please provide all the ranges for the answer");
				return;
			}
		}
		catch(err)
		...
		data.correct_answer= body.range_start+"::"+body.range_end ;
		console.log(data.correct_answer);
		delete data.range_start;
		delete data.range_end;
...
<h3 class="w3-black w3-round w3-padding width">{edit?"Edit a":"Create a"} <span class="w3-text-grey"> Integer with Range Question</span></h3>
<div class="width">
	{#if edit}
	...
	<form on:submit|preventDefault={saveQuestion}>
		...
		<hr>
		<h4>Solutions</h4>
		<label for="question_correct">Range Start value</label>
		<input type="text" placeholder="Start" bind:value={body.range_start} required>
		<br>
		<label for="question_correct">Range End value</label>
		<input type="text" placeholder="End" bind:value={body.range_end} required>
