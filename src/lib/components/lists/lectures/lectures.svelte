...
async function handleDelete(lectureId){
		let confirmm = confirm("Really want to remove the lecture from the batch?");
		if(!confirmm)
		{
			return;
		}
		...
		let res;
		res = await fetch(loginPath+'/panel/batches/batch_remove_lecture/'+batchId+"/"+lectureId,{mode:'cors',method:'post',headers:{'Authorization':'Bearer '+token,'Content-Type':'application/json'}});
