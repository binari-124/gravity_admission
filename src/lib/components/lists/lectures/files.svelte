<script>
  ...
  //           // console.log(__dirname);
	...
	// 	// var token = localStorage.getItem("token");
	// 	const res = await fetch(loginPath+'/files/file_delete/'+fileId,{mode:'cors',method:'get',headers:{'Authorization':'Bearer '+token}});
	// 	if(res.status==200){
	// 		try{
  ...
  async function handleDelete(fileId){
		...
		var loginPath=get(ApiUrl);
		let res;
		res = await fetch(loginPath+'/panel/batches/lecture_remove_file/'+lectureId+"/"+fileId,{mode:'cors',method:'post',headers:{'Authorization':'Bearer '+token,'Content-Type':'application/json'}});
    ...
    <table class="w3-table-all w3-hoverable">
        ...
        <tr class="w3-hover-shadow">
            ...
            <td><a class="w3-text-white w3-round w3-red w3-margin w3-padding w3-card" on:click={handleDelete(file._id)}>Remove</a></td>
        </tr>
        {/each}
      </table>
