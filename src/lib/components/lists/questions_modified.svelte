<script>
  ...
  async function handleDelete(questionId) {
    ...
    let res;
    res = await fetch(loginPath + "/panel/question_delete/" + questionId, {
      mode: "cors",
      method: "post",
      headers: {
    ...
    <table class="w3-table-all w3-hoverable" width="60%">
      ...
      {#each questions as question}
        <tr class="w3-hover-shadow">
          ...
          <td
            ><a class="w3-text-blue" href="/questions/edit/{question._id}"
              >{question._id}</a
            ></td
          >
          <td
    ...
    <table class="w3-table-all w3-hoverable" width="60%">
      ...
      {#each questions as question}
        <tr class="w3-hover-shadow">
          ...
          {#if selectedQuestions.includes(question._id)}
            <td>
              ...
              <a
                ...
                on:click={toggleSelectedQuestions(question._id)}
              >
                Remove</a
              >
            </td>
          ...
          <td class="overflow-scroll" style="max-width:200px">{@html atob(question.question)}</td>
          <td>{question.question_type}</td>
          <!-- <td>{question.}</td> -->
          ...
          <td
            ><a
              class="w3-text-blue"
              target="_blank"
              href="/questions/edit/{question._id}">{question._id}</a
            ></td
          >
        </tr>
      {/each}
