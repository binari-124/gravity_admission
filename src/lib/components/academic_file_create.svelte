<script>
	...
	async function handleSubmit(){
		...
		var loginPath=get(ApiUrl);
		let res;
		res = await fetch(loginPath+'/panel/institute_files/academic_file_create',{mode:'cors',method:'post',headers:{'Authorization':'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify(body)});
...
<h4 class=" w3-round w3-margin w3-xlarge"> Create an Academic File</h4>
...
<form on:submit|preventDefault={handleSubmit} >
	...
	<label>File Id</label>
	<input type="text" class="w3-ipnut w3-block w3-border w3-margin w3-padding w3-round" bind:value={body.file} placeholder="File Id" /><br>

	<label>Description</label>
	<input class="w3-input w3-border w3-round w3-margin w3-padding" type="text" bind:value={body.description} placeholder="Faculties" required/>
    ...
    <hr>
		...
		<select id="question_correct" class="w3-input w3-border w3-round w3-margin" bind:value={body.content_type}>
			<option value="dpp">DPP</option>
			<option value="module">Module</option>
			<option  value="question_paper">Question Paper</option>
			<option value="lecture_document">Lecture Document</option>
